    ...
    for x in centroid_range:
        for y in centroid_range:
            for r in range(r_min, r_max + 1):
                ...
                number_of_match = circle_matching(
                    mask_cir, res_inrange_inv, x, y, r)
...
def main():
    print('>' * 10, 'Crop rectangle from the image(s)', '<' * 10, '\n')
    ...
    for s in symbol_list:
        for j in date:
            ...
            if res is None:
                print('Cannot crop rectangle from the image(s): ', img_name)
                continue

            if (not overwrite) and (not cv.imread(CONST.IMG_RECT_PATH + img_name, 1) is None):
            ...
            cv.imwrite(CONST.IMG_RECT_PATH + img_name, res)
